...
def launch_setup(context: LaunchContext, *args, **kwargs) -> list[LaunchDescriptionEntity]:
    ...
    armed_param = ParameterValue(enable_motion, value_type=bool)

    # Force hardware mode for this launch file.
    set_hw_cfg = [
        SetLaunchConfiguration("use_gazebo", "false"),
    ...
    storm_planner_node = Node(
        ...
        parameters=[storm_yaml, {"armed": armed_param}],
        output="screen",
    ...
    task_node = Node(
        ...
        parameters=[task_yaml, {"armed": armed_param, "auto_start": armed_param}],
        output="screen",
    )
...
def generate_launch_description():
    declared_arguments = [
        DeclareLaunchArgument(
            "enable_motion",
            default_value="false",
            description="Safety gate. When false, the planner publishes a disarmed control policy and the task manager does not command the gripper.",
            choices=["true", "false"],
        ),
        DeclareLaunchArgument(
            "with_rviz",
